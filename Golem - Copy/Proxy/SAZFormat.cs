...
[assembly: RequiredVersion("2.3.7.0")]
    ...
    public class SAZFormat : ISessionImporter, ISessionExporter
    {
        ...
        private static Session[] ReadSessionArchive(string sFilename, bool bVerboseDialogs)
        {
            ...
            */
            ...
            {
                FiddlerApplication.Log.LogString("SAZFormat> ReadSessionArchive Failed. File " + sFilename +
                                                 " does not exist.");
                return null;
            }
            ...
            try
            {
                ...
                {
                    // Sniff for 'PK'
                    FiddlerApplication.Log.LogString("SAZFormat> ReadSessionArchive Failed. " + sFilename +
                                                     " is not a Fiddler-generated .SAZ archive of HTTP Sessions.");
                    oSniff.Close();
                    return null;
                }
                oSniff.Close();
                ...
                if (!oZip.EntryFileNames.Contains("raw/"))
                {
                    FiddlerApplication.Log.LogString(
                        "SAZFormat> ReadSessionArchive Failed. The selected ZIP is not a Fiddler-generated .SAZ archive of HTTP Sessions.");
                    return null;
                }
                ...
                foreach (ZipEntry oZE in oZip)
                {
                    ...
                    try
                    {
                        ...
                        {
                            DiagnosticLog.WriteLine("Incorrect password.");
                    ...
                    {
                        FiddlerApplication.Log.LogString(
                            "SAZFormat> ReadSessionArchive incomplete. Invalid data was present in session: " +
                            oZE.FileName + ".\n\n\n" + eX.Message + "\n" + eX.StackTrace);
                    }
                }
                /*    RetryWithPassword:
        ...
        private static bool WriteSessionArchive(string sFilename, Session[] arrSessions, string sPassword,
            bool bVerboseDialogs)
        {
            if ((null == arrSessions || (arrSessions.Length < 1)))
            {
                ...
                {
                    FiddlerApplication.Log.LogString(
                        "WriteSessionArchive - No Input. No sessions were provided to save to the archive.");
                }
                return false;
            }
            ...
            try
            {
                ...
                oZip.Comment = CONFIG.FiddlerVersionInfo + " " + GetZipLibraryInfo() +
                               " Session Archive. See http://www.fiddler2.com";
                ...
                foreach (Session oSession in arrSessions)
                {
                    ...
                    oZip.AddEntry(sRequestFilename,
                        delegate(string sn, Stream strmToWrite)
                        {
        ...
        private static void oZip_ZipError(object sender, ZipErrorEventArgs e)
        {
        ...
        public bool ExportSessions(string sExportFormat, Session[] oSessions, Dictionary<string, object> dictOptions,
            EventHandler<ProgressCallbackEventArgs> evtProgressNotifications)
        {
            ...
            if (string.IsNullOrEmpty(sFilename))
                sFilename = Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory)
